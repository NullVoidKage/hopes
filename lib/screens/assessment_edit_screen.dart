...
class _AssessmentEditScreenState extends State<AssessmentEditScreen> {
  ...
  void _initializeForm() {
    ...
    _selectedSubject = assessment.subject;
    _isPublished = assessment.isPublished;
    _selectedTags = List.from(assessment.tags);
    // Time limit removed - always set to 0 (no limit)
    _totalPoints = assessment.totalPoints;
    _dueDate = assessment.dueDate;
    _questions = assessment.questions.map((q) => q.copyWith()).toList();
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Modify your assessment: ${widget.assessment.title}',
                  style: TextStyle(
                    fontSize: 16,
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      await assessmentService.updateAssessment(updatedAssessment);

      // Log the activity for teacher dashboard
      await _logTeacherActivity('Assessment Updated', 'Updated assessment: ${updatedAssessment.title}');
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error updating assessment: ${e.toString()}'),
            backgroundColor: Colors.red,
          ),
        );
      }
